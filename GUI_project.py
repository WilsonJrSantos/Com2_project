...
class mainGUI:
    def __init__(self):
        window = tk.Tk()
        window.title("Chat Serial, COM2 USAC 2022")
        self.uartState = False # si no hay puertos series

        # un marco contiene la información de COM y el botón de inicio/parada
        frame_COMinf = tk.Frame(window)
        frame_COMinf.grid(row = 1, column = 1)

        labelCOM = tk.Label(frame_COMinf,text="COMx: ")
        self.COM = tk.StringVar(value = "COM11")
        ...
        labelOutText = tk.Label(frameRecv,text="Chat, mensajes recibidos:")
        ...
        labelInText = tk.Label(frameTrans,text="Para transmitir datos:")
        ...
        self.InputText.pack()
        self.buttonSend = tk.Button(frameTrans, text = "Enviar", command = self.processButtonSend)
        ...
        while True:
            if (self.uartState):
                try:
                    ...
                    InformWindow(infromStr)
                    self.ser.close() # cierra la conexion serial
                    self.buttonSS["text"] = "Start"
                    self.uartState = False
